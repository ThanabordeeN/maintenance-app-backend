  ...
  try {
    ...
    const downtimeStats = await pool.query(`
      SELECT 
        ...
        COALESCE(AVG(downtime_minutes), 0) / 60.0 as avg_downtime_hours,
        COUNT(CASE WHEN downtime_minutes > 0 THEN 1 END) as records_with_downtime
      FROM maintenance_records mr
      WHERE mr.created_at >= $1 AND mr.created_at <= $2::date + interval '1 day'
        AND mr.status = 'completed'
      ${equipmentFilter}
    ...
    const topEquipment = await pool.query(`
      SELECT 
        ...
        COALESCE(SUM(mr.downtime_minutes), 0) / 60.0 as total_downtime
      FROM equipment e
    ...
    const monthlyTrend = await pool.query(`
      SELECT 
        ...
        COUNT(*) as count,
        COALESCE(SUM(total_cost), 0) as total_cost,
        COALESCE(SUM(downtime_minutes), 0) / 60.0 as total_downtime
  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        COALESCE(SUM(mr.downtime_minutes), 0) / 60.0 as total_repair_hours,
        CASE 
          WHEN COUNT(mr.id) > 0 THEN (COALESCE(SUM(mr.downtime_minutes), 0) / 60.0) / COUNT(mr.id)
          ELSE 0
        END as mttr_hours
      FROM equipment e
      LEFT JOIN maintenance_records mr ON e.id = mr.equipment_id
        AND mr.status = 'completed'
        AND mr.downtime_minutes > 0
        AND mr.created_at >= $1 AND mr.created_at <= $2::date + interval '1 day'
      WHERE e.is_active = true
    `;
  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        COALESCE(SUM(mr.downtime_minutes), 0) / 60.0 as downtime_hours,
        $3::numeric - (COALESCE(SUM(mr.downtime_minutes), 0) / 60.0) as operating_hours,
        CASE 
          WHEN $3::numeric > 0 THEN (($3::numeric - (COALESCE(SUM(mr.downtime_minutes), 0) / 60.0)) / $3::numeric) * 100
          ELSE 100
    ...
    const overall = await pool.query(`
      SELECT 
        ...
        CASE 
          ...
          THEN (($3::numeric * COUNT(DISTINCT e.id) - (COALESCE(SUM(mr.downtime_minutes), 0) / 60.0)) / ($3::numeric * COUNT(DISTINCT e.id))) * 100
          ELSE 100
  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        mr.parts_cost,
        mr.total_cost,
        mr.downtime_minutes / 60.0 as downtime_hours,
        e.equipment_name,
        e.equipment_code,
        e.location,
