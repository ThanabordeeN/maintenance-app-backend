...
router.put('/preferences/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const {
      ...
      notify_overdue, notify_low_stock
    } = req.body;
    ...
    const result = await pool.query(`
      ...
      ON CONFLICT (user_id) DO UPDATE SET
        ...
        notify_low_stock = COALESCE($9, notification_preferences.notify_low_stock),
        updated_at = NOW()
      RETURNING *
    `, [
...
export async function checkAndNotifyLowStock() {
  try {
    ...
    if (lowStock.rows.length === 0) return { count: 0 };
    ...
    for (const admin of admins.rows) {
      for (const part of lowStock.rows) {
        await createNotification({
          user_id: admin.id,
          title: `อะไหล่ใกล้หมด: ${part.part_name}`,
          message: `คงเหลือ ${part.current_stock} ${part.unit} (ต่ำสุด: ${part.min_stock_level})`,
          type: 'warning',
          category: 'parts',
    ...
    return { count: lowStock.rows.length };
  } catch (error) {
